...
const repeatGoals = [...goals].filter((goal, index, self) => goal > 0 && self.indexOf(goal) !== index);
const minimalGoals = repeatGoals.sort().at(0);
const repeatMinimalGoals = [...goals].filter((goal) => goal === minimalGoals);
const numbersGames = [...goals].indexOf(repeatMinimalGoals) + 1;
console.log(repeatMinimalGoals);
alert(`Самые не результативные матчи были под номерами ${repeatMinimalGoals}. В каждом из них было забито по ${minimalGoals} мячу(а).`)
